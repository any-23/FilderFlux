...
from filderflux.commands.version.version import cli_version, handle_version
...
def test_handle_version_installed(capsys):
    with patch("filderflux.commands.version.version.cli_version", return_value=MOCK_VERSION):
        args = argparse.Namespace()
        handle_version(args)
        captured = capsys.readouterr()
        assert captured.out.strip() == f"Version of filderflux is {MOCK_VERSION}."


def test_handle_version_not_installed(capsys):
    with patch("filderflux.commands.version.version.cli_version", return_value=""):
        args = argparse.Namespace()
        handle_version(args)
        captured = capsys.readouterr()
        assert captured.out.strip() == "Package is not installed."
